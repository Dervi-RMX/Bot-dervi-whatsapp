...
function buildOutboundPayload(filePath, meta = {}) {
  ...
  if (kind === 'image') {
    return { image: buffer, caption: meta.caption || '', viewOnce: Boolean(meta.viewOnce) };
  }
  if (kind === 'video') {
    return { video: buffer, caption: meta.caption || '', viewOnce: Boolean(meta.viewOnce) };
  }
  if (kind === 'audio') {
    return { audio: buffer, mimetype: mime || undefined, ptt: Boolean(meta.ptt), viewOnce: Boolean(meta.viewOnce) };
  }
  if (kind === 'sticker') {
    return { sticker: buffer, viewOnce: Boolean(meta.viewOnce) };
  }
  return {
    document: buffer,
    ...
    viewOnce: Boolean(meta.viewOnce)
  };
}
