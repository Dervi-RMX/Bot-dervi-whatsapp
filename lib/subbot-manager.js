...
const { downloadQuotedMedia } = require('./downloader');
const { buildOutboundPayload, inferOutboundKindFromMime } = require('./media');
const { getMessageText, getContentType, getMediaInfo } = require('./utils');
...
class SubbotManager {
  ...
  async startInternal(id, options = {}) {
    ...
    runtime.messageCache = new Map();
    socket.ev.on('messages.upsert', ({ messages, type }) => {
      if (type !== 'notify') return;
      for (const message of messages || []) {
        this.cacheMessage(runtime, message);
        this.handleDelete(runtime, message).catch(error => {
          console.warn(`[${record.id}] Error recuperando mensaje eliminado:`, error.message);
        });
        this.dispatch(runtime, message).catch(error => {
  ...
  getCacheKey(key = {}) {
    return `${key.remoteJid || ''}|${key.participant || key.participantPn || ''}|${key.id || ''}`;
  }

  cacheMessage(runtime, message) {
    if (!message?.message || message.message.protocolMessage) return;
    const key = this.getCacheKey(message.key);
    if (!message.key?.id || runtime.messageCache.has(key)) return;
    runtime.messageCache.set(key, message);
    const cutoff = Date.now() - 30 * 60 * 1000;
    for (const [cacheKey, cached] of runtime.messageCache) {
      if (Number(cached.messageTimestamp || 0) * 1000 < cutoff) runtime.messageCache.delete(cacheKey);
    }
  }

  async handleDelete(runtime, message) {
    if (!message?.message?.protocolMessage?.key?.id) return;
    const deletionKey = message.message.protocolMessage.key;
    const original = runtime.messageCache.get(this.getCacheKey(deletionKey))
      || [...runtime.messageCache.values()].find(item => item.key?.id === deletionKey.id);
    if (!original) return;
    const target = normalizeJid(runtime.socket.user?.id || runtime.socket.user?.lid || '');
    if (!target) return;
    const content = original.message || {};
    const type = getContentType(content);
    if (!type) return;
    const text = getMessageText(original) || 'Contenido eliminado';
    if (!['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(type)) {
      await runtime.socket.sendMessage(target, { text: `🕵️ Mensaje eliminado en ${original.key?.remoteJid || 'chat'}:\n${text}` });
      return;
    }
    try {
      const filePath = await downloadQuotedMedia(original, runtime.handler.config.tempDirectory);
      const info = getMediaInfo(original) || {};
      await runtime.socket.sendMessage(target, buildOutboundPayload(filePath, {
        fileName: info.fileName || 'archivo',
        mimeType: info.mimetype || '',
        kind: inferOutboundKindFromMime(info.mimetype || ''),
        caption: `🕵️ Contenido eliminado en ${original.key?.remoteJid || 'chat'}`
      }));
      await fs.promises.unlink(filePath).catch(() => null);
    } catch (error) {
      await runtime.socket.sendMessage(target, { text: `🕵️ Se eliminó contenido multimedia en ${original.key?.remoteJid || 'chat'}, pero no se pudo recuperar.` });
    }
  }
