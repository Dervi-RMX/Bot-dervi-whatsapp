...
class ModerationManager {
  constructor(config) {
    ...
    this.filePath = this.store.path('moderation.json');
    this.loaded = false;
  }
  ...
  load() {
    ...
    try {
      const parsed = this.store.read('moderation.json', this.state);
  ...
  save() {
    this.load();
    ensureDir(this.config.dataDirectory);
    this.store.write('moderation.json', this.state);
  }
  ...
  ensureChat(chatId) {
    ...
    if (!chat.antiCommand || typeof chat.antiCommand !== 'object') chat.antiCommand = { enabled: false };
    chat.antiCommand.enabled = Boolean(chat.antiCommand.enabled);
  ...
  getAntiCommand(chatId) {
    const chat = this.ensureChat(chatId);
    return { enabled: Boolean(chat.antiCommand.enabled) };
  }

  setAntiCommand(chatId, patch = {}) {
    const chat = this.ensureChat(chatId);
    chat.antiCommand = {
      ...chat.antiCommand,
      enabled: patch.enabled === undefined
        ? Boolean(chat.antiCommand.enabled)
        : Boolean(patch.enabled)
    };
    this.save();
    return this.getAntiCommand(chatId);
  }
  ...
  getWelcome(chatId) {
    ...
    const result = {
      ...
      photoUrl: chat.welcome.photoUrl ?? null
    };
  ...
  setWelcome(chatId, patch = {}) {
    ...
    const nextPhotoUrl = patch.photoUrl === undefined
      ? chat.welcome.photoUrl
      : String(patch.photoUrl || '').trim() || null;
