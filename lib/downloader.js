...
async function downloadWithYtDlp(url, tempDir, options = {}) {
  ...
  // resolve yt-dlp binary: on Windows, try bundled yt-dlp.exe; on non-Windows, rely on PATH
  let ytdlpCmd = 'yt-dlp';
  ...
  // On non-Windows, we leave ytdlpCmd as 'yt-dlp' and rely on it being in PATH
  ...
  const jsRuntime = options.jsRuntimes ? options.jsRuntimes : ['jSEngine'];
