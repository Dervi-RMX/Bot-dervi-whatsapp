...
module.exports = {
  ...
  description: 'Resume textos usando IA: .resumen <texto> or responder a un mensaje con .resumen',
  async execute(context) {
    ...
    if (!prompt) {
      await context.reply(`⚠️ Usa: ${context.prefix}resumen <texto>\nTambién puedes responder a un mensaje y escribir ${context.prefix}resumen`);
      return;
    }
    ...
    for (const provider of providers) {
      try {
        ...
        const summaryPrompt = `Resume el siguiente texto de manera concisa y clara en español:\n\n${prompt}`;

        const answer = await provider.request(summaryPrompt, config);
